...
public class RepositoryFileProxy extends RepositoryFile {
  ...
  public RepositoryFileProxy(final Node node, final JcrTemplate template, IPentahoLocale pentahoLocale){
    super(null, null, false, false, false, null,null, null,null, false, null,null, null,null, null, null,null, null,-1, null, null);
    this.node = node;
          ...
          try {
            if (JcrRepositoryFileUtils.isPentahoHierarchyNode(session, getPentahoJcrConstants(), node)) {
              ...
              if (title == null && node.hasNode(getPentahoJcrConstants().getPHO_TITLE())) {
                title = JcrRepositoryFileUtils.getLocalizedString(session, getPentahoJcrConstants(), node.getNode(getPentahoJcrConstants().getPHO_TITLE()),
                    pentahoLocale);
              }
              if (description == null && node.hasNode(getPentahoJcrConstants().getPHO_DESCRIPTION())) {
                description = JcrRepositoryFileUtils.getLocalizedString(session, getPentahoJcrConstants(),
                    node.getNode(getPentahoJcrConstants().getPHO_DESCRIPTION()), pentahoLocale);
              }
    ...
    if(localeMap == null){
      this.executeOperation(new SessionOperation() {
        ...
        public void execute(Session session) {
          ...
          try {
            localeMap = JcrRepositoryFileUtils.getLocalePropertiesMap(session, getPentahoJcrConstants(),
                node.getNode(getPentahoJcrConstants().getPHO_LOCALES()));
          } catch (RepositoryException e) {
            e.printStackTrace();  //To change body of catch statement use File | Settings | File Templates.
          }
        }
  ...
  public String getName() {
    if(name == null){
      this.executeOperation(new SessionOperation() {
        ...
        public void execute(Session session) {
          try {
            name = RepositoryFile.SEPARATOR.equals(getPath()) ? "" : JcrRepositoryFileUtils.getNodeName(session, getPentahoJcrConstants(), node); //$NON-NLS-1$
          } catch (RepositoryException e) {
            e.printStackTrace();  //To change body of catch statement use File | Settings | File Templates.
          }
        }
  ...
  public String getOriginalParentFolderPath() {
    return super.getOriginalParentFolderPath();    //To change body of overridden methods use File | Settings | File Templates.
  }
  ...
  public String getPath() {
    if(path == null){
      this.executeOperation(new SessionOperation() {
        ...
        public void execute(Session session) {
          try {
            path = new DefaultPathConversionHelper().absToRel((JcrRepositoryFileUtils.getAbsolutePath(session, getPentahoJcrConstants(), node)));
          } catch (RepositoryException e) {
            e.printStackTrace();  //To change body of catch statement use File | Settings | File Templates.
          }
        }
