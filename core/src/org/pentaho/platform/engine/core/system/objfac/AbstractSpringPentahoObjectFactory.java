...
public abstract class AbstractSpringPentahoObjectFactory implements IPentahoObjectFactory {
  ...
  public <T> List<T> getAll(Class<T> interfaceClass, IPentahoSession curSession) throws ObjectFactoryException {
    return (List<T>) retreiveObjects(interfaceClass, curSession);
  }
  ...
  private BeanDefinition getBeanDefinitionFromFactory(final String name) {
    if(beanFactory instanceof BeanDefinitionRegistry){
      return ((BeanDefinitionRegistry) beanFactory).getBeanDefinition(name);
    } else if(beanFactory instanceof ConfigurableListableBeanFactory){
      return ((ConfigurableListableBeanFactory) beanFactory).getBeanDefinition(name);
    } else if(beanFactory instanceof AbstractRefreshableApplicationContext){
      return ((AbstractRefreshableApplicationContext) beanFactory).getBeanFactory().getBeanDefinition(name);
    }
    logger.warn("ApplicationContext implementation does not support BeanDefinition lookups");
    return null;
  }
  ...
  protected void setBeanFactory(ApplicationContext context){
    beanFactory = context;
  }
