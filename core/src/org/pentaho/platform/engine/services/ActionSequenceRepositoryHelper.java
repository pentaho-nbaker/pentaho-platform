 ...
 *
 */
public class ActionSequenceRepositoryHelper {
  protected static final String PROPERTIES_SUFFIX = ".properties"; //$NON-NLS-1$
  protected static final Log logger = LogFactory.getLog(ActionSequenceRepositoryHelper.class);
  
  private IPentahoSession pentahoSession;
  private IUnifiedRepository repository;
  
  public ActionSequenceRepositoryHelper() {
    this (PentahoSessionHolder.getSession());
  }
  
  public ActionSequenceRepositoryHelper(IPentahoSession pentahoSession) {
    super();
    ...
    if (repository == null) {
      final String errorMessage = Messages.getInstance().getErrorString("ActionSequenceRepositoryHelper.ERROR_0001_INVALID_REPOSITORY"); //$NON-NLS-1$
  ...
  public IActionSequence getActionSequence(String actionPath, int loggingLevel, int actionOperation) {
    ...
    IActionSequence actionSequence = SequenceDefinition.ActionSequenceFactory(actionSequenceDocument,
        actionPath, pentahoSession, PentahoSystem.getApplicationContext(), loggingLevel);
  ...
  public Document getSolutionDocument(final String documentPath, final int actionOperation) {
    ...
    if (data != null) {
      try {
        ...
        logger.error(Messages.getInstance().getErrorString("ActionSequenceRepositoryHelper.ERROR_0017_INVALID_XML_DOCUMENT", documentPath), t); //$NON-NLS-1$
        return null;
      }
    } else {
      logger.error(Messages.getInstance().getErrorString("ActionSequenceRepositoryHelper.ERROR_0019_NO_DATA_IN_FILE", file.getName())); //$NON-NLS-1$
      return null;
    }
    if ((document == null) && (file != null) && (data != null)) {
      // the document exists but cannot be parsed
      logger.error(Messages.getInstance().getErrorString("ActionSequenceRepositoryHelper.ERROR_0009_INVALID_DOCUMENT", documentPath)); //$NON-NLS-1$
      return null;
    }
      ...
      while (nodeIterator.hasNext()) {
        ...
        if (name.startsWith("%") && !node.getPath().endsWith("/text()")) { //$NON-NLS-1$ //$NON-NLS-2$
          try {
            ...
            logger.warn(Messages.getInstance().getString("ActionSequenceRepositoryHelper.WARN_MISSING_RESOURCE_PROPERTY", name.substring(1), baseName, getLocale().toString())); //$NON-NLS-1$
          }
        }
      }
    } catch (Exception e) {
      logger.error(Messages.getInstance().getErrorString("ActionSequenceRepositoryHelper.ERROR_0007_COULD_NOT_READ_PROPERTIES", file.getPath()), e); //$NON-NLS-1$
    }
  }
  ...
  protected String getLocaleString(final String key, String baseName, final RepositoryFile baseFile, boolean marchUpParents) {
    ...
    try {
      ...
      logger.error(Messages.getInstance().getErrorString("ActionSequenceRepositoryHelper.ERROR_0007_COULD_NOT_READ_PROPERTIES", baseFile.getPath()), e); //$NON-NLS-1$
    }
    return null;
  }
