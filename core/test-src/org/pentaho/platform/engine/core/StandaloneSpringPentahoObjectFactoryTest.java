...
public class StandaloneSpringPentahoObjectFactoryTest extends TestCase {
  ...
  public void testCombined() throws Exception {
    ...
    f = new File("test-res/solution/system/pentahoObjects-ext.spring.xml");
    fsr = new FileSystemResource(f);
    GenericApplicationContext appCtx2 = new GenericApplicationContext();
    XmlBeanDefinitionReader xmlReader2 = new XmlBeanDefinitionReader(appCtx2);
    xmlReader2.loadBeanDefinitions(fsr);

    String[] names = appCtx2.getBeanDefinitionNames();
    for(String name : names){
      BeanDefinition def = appCtx2.getBeanDefinition(name);

      appCtx.registerBeanDefinition(name, def);
    }
    ...
    IContentInfo obj = factory.get(IContentInfo.class, session);
    assertEquals("Test Secondary Context", obj.getTitle());
